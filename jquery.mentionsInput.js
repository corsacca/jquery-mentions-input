...
(function ($, undefined) {
	...
	var defaultSettings = {
		...
		templates     : {
			wrapper                    : function() {
				return '<div class="mentions-input-box"></div>';
			},
			autocompleteList           : function() {
				return '<div class="mentions-autocomplete-list"></div>';
			},
			autocompleteListItem       :  function(data) {
				return "<li data-ref-id="+data.id+" data-ref-type="+data.type+" data-display="+data.display+">"+data.content+"</li>";
			},
			autocompleteListItemAvatar : function(data) {
				return "<img src="+data.avatar+" />";
			},
			autocompleteListItemIcon   :  function(data) {
				return '<div class="icon"'+data.icon+'></div>';
			},
			mentionsOverlay            : function(data) {
				return '<div class="mentions"><div></div></div>';
			},
			mentionItemSyntax          : function(data) {
				return '@['+data.value+']('+data.type+':'+data.id+')'
			},
			mentionItemHighlight       : function(data) {
				return '<strong><span>'+data.value+'</span></strong>';
			}
		}
	};
	...
	var MentionsInput = function (settings) {
		...
		function updateMentionsCollection() {
			...
			mentionsCollection = mentionsCollection.filter(function (mention) {
				return !(!mention.value || inputText.indexOf(mention.value) == -1);
			});
			mentionsCollection = mentionsCollection.filter(Boolean);
		}
		...
		function addMention(mention) {
			...
			if( !mentionsCollection.find(function (object) { return object.id == mention.id; })) {
